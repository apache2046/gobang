    ...
    def search(self, state, stones: int, level=0):
        ...
        if ns[sk] == 0:
            ...
            if self.random_dihedral_reflection:
                ...
                # if cached := self.rdb.get(sk):
                #     v = np.frombuffer(cached[:2], dtype=np.float16)
                #     p = np.frombuffer(cached[2:], dtype=np.uint8).astype(np.float32)
                #     p /= 255.0
                # else:
                #     p, v = yield transform_state
                #     print("G123", v, len(v.tobytes()), p, len(p.tobytes()))
                #     self.rdb.set(sk, v.astype(np.float16).tobytes() + np.round(p * 255.0).astype(np.uint8).tobytes())
                p, v = yield transform_state
